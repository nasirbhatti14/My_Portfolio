...
export default function Certifications() {
  return (
    <section id="certifications" className="py-12 md:py-16 relative overflow-hidden bg-zinc-900">
      {/* Glow elements */}
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {certifications.map((cert) => (
            <div 
              key={cert.id}
              className="p-6 md:p-7 rounded-2xl glass-panel border border-violet-500/10 hover:border-violet-500/25 transition-all duration-300 flex flex-col justify-between relative group overflow-hidden"
            >
              ...
              <div>
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="p-3.5 rounded-xl bg-violet-600/10 border border-violet-500/20 text-violet-400 shrink-0">
                    <Award className="w-6 h-6" />
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-[11px] font-mono text-violet-400 font-semibold uppercase tracking-wider bg-violet-950/50 border border-violet-900/30 px-2.5 py-0.5 rounded-full">
                      Verified
                    </span>
                ...
                {cert.skills && cert.skills.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-4 mt-2 border-t border-zinc-800/60">
                    {cert.skills.map((skill, idx) => (
                      <span key={idx} className="text-xs text-zinc-400 flex items-center gap-1.5 font-sans">
